...
import { Calendar as CalendarIcon, MapPin, Users, Bell, CheckCircle, Activity } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  return (
    <div>
      ...
      {/* RSVP Real-Time Tracker for closest event */}
      {closestEvent && (
        <div className="card" style={{ marginBottom: '2rem', border: '2px solid var(--color-blue-main)' }}>
          <h3 style={{ marginBottom: '1rem', color: 'var(--color-blue-dark)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <CheckCircle size={20} /> מעקב הגעה בזמן אמת: {closestEvent.title}
          </h3>
          <div style={{ display: 'flex', gap: '2rem', alignItems: 'center' }}>
            <div style={{ flex: 1, background: 'var(--color-bg-main)', padding: '1rem', borderRadius: '8px', textAlign: 'center' }}>
              <div style={{ fontSize: '2rem', fontWeight: 'bold', color: 'var(--color-success)' }}>120</div>
              <div style={{ color: 'var(--color-text-light)' }}>אישרו הגעה (Confirmed)</div>
            </div>
            <div style={{ flex: 1, background: 'var(--color-bg-main)', padding: '1rem', borderRadius: '8px', textAlign: 'center' }}>
              <div style={{ fontSize: '2rem', fontWeight: 'bold', color: 'var(--color-blue-main)' }}>{closestEvent.estimatedHeadcount || 150}</div>
              <div style={{ color: 'var(--color-text-light)' }}>סה"כ מוזמנים (Total Invited)</div>
            </div>
            <div style={{ flex: 1, background: 'var(--color-bg-main)', padding: '1rem', borderRadius: '8px', textAlign: 'center' }}>
              <div style={{ fontSize: '2rem', fontWeight: 'bold', color: 'var(--color-warning)' }}>5</div>
              <div style={{ color: 'var(--color-text-light)' }}>רשימת המתנה</div>
            </div>
          </div>
        </div>
      )}
          ...
          {upcomingEvents.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {upcomingEvents.slice(0, 4).map(event => (
                <div key={event.id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
                     ...
                     {event.status !== 'completed' && (
                       <button className="btn btn-primary" style={{ flex: 1, padding: '0.25rem', fontSize: '0.85rem', background: 'var(--color-success)', borderColor: 'var(--color-success)', color: 'white' }} onClick={() => alert('אירוע סומן כהסתיים!')}>
                         סיום אירוע
                       </button>
                     )}
                  </div>
